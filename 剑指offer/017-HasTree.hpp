...
bool isSameTree(BinaryTreeNode* pRoot1, BinaryTreeNode* pRoot2)
{
    ...
    // if current nodes are equal
    if (pRoot1->m_nValue == pRoot2->m_nValue)
    {
        bool hasLeft = isSameTree(pRoot1->m_pLeft, pRoot2->m_pLeft);
        bool hasRight = isSameTree(pRoot1->m_pRight, pRoot2->m_pRight);
        if (hasLeft && hasRight)
        {
            return true;
        }
...
// to determine if tree 2 is a sub-tree of tree 1
bool HasSubtree(BinaryTreeNode* pRoot1, BinaryTreeNode* pRoot2)
{
    ...
    // a tree2's root has found
    // check if their left and right tree are the same
    if (pRoot1->m_nValue == pRoot2->m_nValue)
    {
        result = isSameTree(pRoot1, pRoot2);
    }

    // if not, try to find a the root node of tree2
    if (result == false)
    {
